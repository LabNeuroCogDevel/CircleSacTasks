...
ncfile<-Sys.glob('workingMemory_noCatch/stims/*/eff.txt')
ncsfile<-Sys.glob('workingMemory_noCatchShort/stims/*/eff.txt')
...
print(p)
p<-ggplot(data=stds[grep('-.*LC$',stds$contrast),], aes(x=dly,y=power,color=dly)) + geom_point()  +geom_violin() + facet_wrap(~contrast,scales="free_y") + theme_bw() + ggtitle('power')
print(p)
```
