...
nc <- adply(ncfile,1,function(x){read.table(header=F,x,sep="\t")[,c(1,2,4)]})[2:4]
so$dly <- "short"
sl$dly <- "both"
nc$dly <- "noCatch"
stds <- rbind(rbind(so,sl),nc);
...
p<-p+theme_bw() +ggtitle('volin plot: dist. of std. dev. of est. of IRF/contrast')
print(p)
```
